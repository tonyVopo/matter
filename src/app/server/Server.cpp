...
void Server::RejoinExistingMulticastGroups()
{
    ...
    for (const FabricInfo & fabric : mFabrics)
    {
        ...
        if (iterator)
        {
            ...
            {
                bool use_iana_addr = !(groupInfo.flags & static_cast<uint8_t>(Credentials::GroupDataProvider::GroupInfo::Flags::kMcastAddrPolicy));
                ...
                if(use_iana_addr) groupcast_joined = true;
            }
